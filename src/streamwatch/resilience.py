...
from . import config
from .exceptions import StreamlinkError, NetworkError, TimeoutError, AuthenticationError
        ...
        for attempt in range(1, self.config.max_attempts + 1):
            try:
                ...
                if not self.should_retry(e, attempt):
                    logger.warning(f"{operation_name} failed on attempt {attempt}, not retrying: {e}")
                    break
                ...
                if attempt < self.config.max_attempts:
                    ...
                    logger.error(f"{operation_name} failed on final attempt {attempt}: {e}")
...
class CircuitBreaker:
    ...
    def _should_attempt_reset(self) -> bool:
        ...
        return (
            self.state.state == CircuitState.OPEN and
            time.time() >= self.state.next_attempt_time
        )
    ...
    def _record_failure(self, exception: Exception):
        ...
        if self.state.state == CircuitState.CLOSED:
            ...
            logger.warning(f"Circuit breaker '{self.name}' reopening after failed recovery attempt")
            self.state.state = CircuitState.OPEN
            self.state.success_count = 0
            self.state.next_attempt_time = time.time() + self.config.recovery_timeout
    ...
    def get_state_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'next_attempt_time': self.state.next_attempt_time if self.state.state == CircuitState.OPEN else None
        }
        ...
        def wrapper(*args, **kwargs) -> T:
            ...
            def execute_operation():
                if use_circuit_breaker:
                    ...
                    return circuit_breaker.execute(lambda: func(*args, **kwargs), operation_name)
                else:

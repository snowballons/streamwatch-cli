...
def parse_url_metadata(url: str) -> Dict[str, str]:
    ...
    Args:
        url: The stream URL to parse

    Returns:
        Dictionary containing platform, username, and type information
    """
    ...
    if "youtube.com" in netloc:
        m = re.match(
            r"/(?:@([a-zA-Z0-9_.-]+)|channel/([a-zA-Z0-9_-]+)|c/([a-zA-Z0-9_.-]+)|user/([a-zA-Z0-9_.-]+))/?",
            parsed_uri.path,
        )
        if m:
            return _result(
        ...
        if path_match:
            ...
            if (
                not video_id and path_match.group(1) == "watch" and parsed_uri.query
            ):  # This is for /watch?v=...
                # Simple query parsing
                query_params = dict(
                    p.split("=", 1) for p in parsed_uri.query.split("&") if "=" in p
                )
    ...
    if "bbc.co.uk" in netloc:
        ...
        if m:
            return _result("BBC iPlayer", m.group(1), "channel_id")

    if "zdf.de" in netloc:
        m = re.match(r"/live-tv(?:/([a-zA-Z0-9_-]+))?/?$", parsed_uri.path)
        if m:
            return _result("ZDF Mediathek", m.group(1) or "zdf", "channel_id")
    ...
    if path_parts:
        ...
        if (
            username_guess.lower() in ["live", "channel", "streams", "videos"]
...
def parse_url_metadata_typed(url: str) -> UrlMetadata:
    ...
    Returns:
        UrlMetadata object containing parsed information
    ...
    # Use the existing function and convert to typed model
